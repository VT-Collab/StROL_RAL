...
from entities import RectangleEntity, CircleEntity, RingEntity, LineEntity
...
class Visualizer:
    ...
    def update_agents(self, agents: list):
        ...
        for imgItem in self.visualized_imgs:
            if imgItem['movable'] or isinstance(imgItem['graphics'], Line):
        ...
        for agent in agents:
            if agent.movable or not self.visualized_imgs or isinstance(agent, LineEntity):
                ...
                elif isinstance(agent, LineEntity):
                    # print(self.ppm*agent.start, self.ppm*agent.end)
                    img = Line(Point(self.ppm*agent.start.x, self.display_height - self.ppm*agent.start.y), Point(self.ppm*agent.end.x, self.display_height - self.ppm*agent.end.y))
                else:
